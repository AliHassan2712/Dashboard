...
export const CompressorModal = ({ isOpen, onClose, onSave }: Props) => {
  ...
  const imageUrl = watch("imageUrl"); // نراقب الصورة لعرضها

  useEffect(() => {
    if (isOpen) reset(); // تصفير عند الفتح
  }, [isOpen, reset]);
        ...
        <div className="space-y-2">
          ...
          {imageUrl ? (
            <div className="relative h-44 w-full rounded-2xl overflow-hidden border-2 border-indigo-100 group">
              ...
              <button type="button" onClick={() => setValue("imageUrl", "")} className="absolute top-2 right-2 bg-rose-500 text-white p-1.5 rounded-full shadow-lg hover:bg-rose-600 transition-all">
                <X className="w-4 h-4" />
              </button>
            </div>
          ) : (
