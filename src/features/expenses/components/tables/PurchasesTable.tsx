    ...
    <table className="w-full text-right text-sm">
      <thead className="bg-gray-50 text-gray-500 font-bold border-b">
        <tr>
          ...
          <th className="p-4 text-emerald-600">المدفوع نقداً (كاش)</th>
          <th className="p-4 text-rose-500">قُيد كدين للحساب</th>
      ...
      <tbody className="divide-y divide-gray-100">
        {purchases.length === 0 ? (
          <tr>
            <td colSpan={8} className="text-center py-10 text-gray-400">لا توجد فواتير مشتريات مسجلة.</td>
          </tr>
        ) : (
          purchases.map(inv => {
            ...
            const statusColor = addedToDebt <= 0 ? "bg-emerald-50 text-emerald-600" : (inv.paidAmount > 0 ? "bg-blue-50 text-blue-600" : "bg-rose-50 text-rose-600");

            return (
              <tr key={inv.id} className="hover:bg-gray-50 transition">
                <td className="p-4 font-bold text-gray-400 font-mono text-xs">#{inv.id.slice(-6).toUpperCase()}</td>
                <td className="p-4">
                  <div className="font-bold text-gray-900">{inv.supplier?.name}</div>
                  <button onClick={() => onOpenLedger(inv.supplierId)} className="text-[10px] text-indigo-500 hover:text-indigo-700 font-bold flex items-center gap-1 mt-1 transition">
                ...
                <td className="p-4 text-xs text-gray-500 max-w-[150px] truncate" title={invoiceNotes}>
                  {invoiceNotes}
                </td>
              </tr>
            )
