...
export const SupplierModal = ({ isOpen, onClose, onBack, onSave }: Props) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={<><UserPlus className="w-5 h-5 text-indigo-600" /> إضافة مورد جديد</>}>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="flex gap-3 pt-2">
          <button type="button" onClick={onBack} className="flex-1 bg-gray-100 text-gray-700 py-3 rounded-xl font-bold transition hover:bg-gray-200">رجوع للفاتورة</button>
          <button disabled={isSubmitting} type="submit" className="flex-1 bg-indigo-600 text-white py-3 rounded-xl font-bold flex justify-center items-center transition hover:bg-indigo-700">
